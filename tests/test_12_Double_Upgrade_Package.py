...
class TestFOTA(BaseClass):
    def test_Double_Upgrade_Package(self):
        try:
            log = self.getLogger()
            self.selectProjectDetails()
            sheet = self.excelPathOf16038()
            log.info("Updating double upgrade package details")
            file_name = sheet['h29'].value
            log.info(file_name)
            file_description = sheet['i29'].value
            log.info(file_description)
            source_version = sheet['c29'].value
            log.info(source_version)
            target_version = sheet['e29'].value
