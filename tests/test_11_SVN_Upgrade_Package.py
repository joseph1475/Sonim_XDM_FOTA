...
class TestFOTA(BaseClass):
    def test_SVN_Upgrade_Package(self):
        try:
            log = self.getLogger()
            self.selectProjectDetails()
            sheet = self.excelPathOf16038()
            log.info("Updating svn upgrade package details")
            file_name = sheet['h28'].value
            log.info(file_name)
            file_description = sheet['i28'].value
            log.info(file_description)
            source_version = sheet['c28'].value
            log.info(source_version)
            target_version = sheet['e28'].value
