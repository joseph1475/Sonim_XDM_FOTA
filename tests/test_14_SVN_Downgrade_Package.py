...
class TestFOTA(BaseClass):
    def test_SVN_Downgrade_Package(self):
        try:
            log = self.getLogger()
            self.selectProjectDetails()
            sheet = self.excelPathOf16038()
            log.info("Updating svn downgrade test package details")
            file_name = sheet['h31'].value
            log.info(file_name)
            file_description = sheet['i31'].value
            log.info(file_description)
            source_version = sheet['c31'].value
            log.info(source_version)
            target_version = sheet['e31'].value
