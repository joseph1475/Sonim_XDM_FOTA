...
class TestFOTA(BaseClass):
    def test_20MB(self):
        try:
            ...
            log.info("Updating 20MB package details")
            file_name = sheet['h21'].value
            log.info(file_name)
            file_description = sheet['i21'].value
            log.info(file_description)
            source_version = sheet['c21'].value
            log.info(source_version)
            target_version = sheet['e21'].value
