...
class TestFOTA(BaseClass):
    def test_Corrupt_Package(self):
        try:
            ...
            log.info("Updating corrupt package details")
            file_name = sheet['h27'].value
            log.info(file_name)
            file_description = sheet['i27'].value
            log.info(file_description)
            source_version = sheet['c27'].value
            log.info(source_version)
            target_version = sheet['e27'].value
