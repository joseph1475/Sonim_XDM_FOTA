...
class TestFOTA(BaseClass):
    def test_250MB(self):
        try:
            ...
            log.info("Updating 250MB package details")
            file_name = sheet['h24'].value
            log.info(file_name)
            file_description = sheet['i24'].value
            log.info(file_description)
            source_version = sheet['c24'].value
            log.info(source_version)
            target_version = sheet['e24'].value
