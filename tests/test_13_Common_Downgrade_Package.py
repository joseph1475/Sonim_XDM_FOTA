...
class TestFOTA(BaseClass):
    def test_Common_Downgrade_Package(self):
        try:
            log = self.getLogger()
            self.selectProjectDetails()
            sheet = self.excelPathOf16038()
            log.info("Updating common downgrade test package details")
            file_name = sheet['h30'].value
            log.info(file_name)
            file_description = sheet['i30'].value
            log.info(file_description)
            source_version = sheet['c30'].value
            log.info(source_version)
            target_version = sheet['e30'].value
