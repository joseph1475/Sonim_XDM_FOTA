...
class TestFOTA(BaseClass):
    def test_extra_package2(self):
        try:
            log = self.getLogger()
            self.selectProjectDetails()
            sheet = self.excelPathOf16038()
            log.info("Updating extra package details")
            file_name = sheet['h8'].value
            log.info(file_name)
            file_description = sheet['i8'].value
            log.info(file_description)
            source_version = sheet['c8'].value
            log.info(source_version)
            target_version = sheet['e8'].value
