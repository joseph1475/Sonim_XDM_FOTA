...
class TestFOTA(BaseClass):
    def test_500MB(self):
        try:
            ...
            log.info("Updating 500MB package details")
            file_name = sheet['h25'].value
            log.info(file_name)
            file_description = sheet['i25'].value
            log.info(file_description)
            source_version = sheet['c25'].value
            log.info(source_version)
            target_version = sheet['e25'].value
